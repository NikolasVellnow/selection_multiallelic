% Case of calculation of fluctuating fitness effects

% Program is for n=3 alleles.  
% It generates a single 'A' matrix for the case with fluctuating selection. 
...
for r=1:Reps
    ...
    for k=1:T
        x=Xd(:,k,r);
        V=diag(x)-x*x';
        A = calc_A_fluctuating(lambda, n);      % Calculate random matrix A each generation
        D=V*A*x/(1+x'*A*x);
        xp=x+D;
